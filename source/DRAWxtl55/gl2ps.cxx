/* $Id: gl2ps.cxx 1079 2010-11-10 23:03:23Z martin $ */
/*
...
extern void Progress_Window (int, const char *, float);
...
#define GL2PS_EPSILON       5.0e-2F
#define GL2PS_ZSCALE        4.0F
#define GL2PS_ZOFFSET       5.0e-1F
#define GL2PS_ZOFFSET_LARGE 2.0F
#define GL2PS_ZERO(arg)     (fabs(arg) < 1.e-20)
...
gl2psSameColor (GL2PSrgba rgba1, GL2PSrgba rgba2)
{
	...
	!GL2PS_ZERO (rgba1[1] - rgba2[1]) || !GL2PS_ZERO (rgba1[2] - rgba2[2]))
	return GL_FALSE;
    return GL_TRUE;
}
...
static GLfloat
gl2psGetRGB (GL2PSimage * im, GLuint x, GLuint y,
...
gl2psSamePosition (GL2PSxyz p1, GL2PSxyz p2)
{
	...
	!GL2PS_ZERO (p1[1] - p2[1]) || !GL2PS_ZERO (p1[2] - p2[2]))
	return GL_FALSE;
    return GL_TRUE;
}
...
static void
gl2psAddIndex (GLshort * index0, GLshort * index1, GLshort * nb, GLshort i, GLshort j)
{
    GLint k;
...
static void
gl2psDivideQuad (GL2PSprimitive * quad, GL2PSprimitive ** t1, GL2PSprimitive ** t2)
{
...
gl2psAddPlanesInBspTreeImage (GL2PSprimitive * prim, GL2PSbsptree2d ** tree)
{
	...
	case GL2PS_POINT_INFRONT:
		...
		if (gl2psGetPlaneFromPoints (prim->verts[i].xyz,
					     prim->verts[i + 1].xyz, cur->front->plane)) {
		    cur = cur->front;
		    cur->front = NULL;
		    cur->back = NULL;
		}
	    ...
	    if (gl2psGetPlaneFromPoints (prim->verts[i].xyz,
					 prim->verts[offset].xyz, cur->front->plane)) {
...
static GL2PSprimitive *
gl2psCreateSplitPrimitive2D (GL2PSprimitive * parent,
			     GLshort numverts, GL2PSvertex * vertx)
{
    GLint i;
...
static void
gl2psSplitPrimitive2D (GL2PSprimitive * prim,
		       GL2PSplane plane, GL2PSprimitive ** front, GL2PSprimitive ** back)
{
...
gl2psAddInBspImageTree (GL2PSprimitive * prim, GL2PSbsptree2d ** tree)
{
	...
	prim->type == GL2PS_TEXT || prim->type == GL2PS_SPECIAL) {
	return 1;
    }
...
gl2psAddBoundaryInList (GL2PSprimitive * prim, GL2PSlist * list)
{
	...
	if (prim->boundary & (GLint) pow (2., i)) {
	    ...
	    v[1] /= norm;
	    b->verts[1].xyz[0] =
		prim->verts[gl2psGetIndex (i, prim->numverts)].xyz[0] + 0.1 * v[0];
	    b->verts[1].xyz[1] =
		prim->verts[gl2psGetIndex (i, prim->numverts)].xyz[1] + 0.1 * v[1];
...
static void
gl2psAddPolyPrimitive (GLshort type, GLshort numverts,
		       GL2PSvertex * verts, GLint offset,
		       GLushort pattern, GLint factor, GLfloat width, char boundary)
{
    GL2PSprimitive *prim;
...
gl2psPrintPostScriptPixmap (GLfloat x, GLfloat y, GL2PSimage * im)
{
	...
	for (row = 0; row < height; row++) {
		...
		grey = (unsigned char) (255.0f * fgrey);
		gl2psWriteByte (grey);
	    }
	    gl2psPrintf ("\n");
	}
...
static void
gl2psPrintPostScriptImagemap (GLfloat x, GLfloat y,
...
static void
gl2psParseStipplePattern (GLushort pattern, GLint factor, int *nb, int array[10])
{
    int i, n;
...
static int
gl2psPrintPostScriptDash (GLushort pattern, GLint factor, const char *str)
{
...
{
    (void)viewport; // mask unused variable
    glRenderMode (GL_FEEDBACK);
...
gl2psPDFgroupListWriteShaderResources (void)
{
	...
	if (gro->maskshno >= 0)
	    offs +=
		fprintf (gl2ps->stream, "/TrSh%d %d 0 R\n", gro->maskshno,
			 gro->maskshobjno);
    }
    offs += fprintf (gl2ps->stream, ">>\n");
    return offs;
...
gl2psPDFgroupListWriteXObjectResources (void)
{
	...
	case GL2PS_TRIANGLE:
	    if (gro->trgroupno >= 0)
		offs +=
		    fprintf (gl2ps->stream, "/TrG%d %d 0 R\n", gro->trgroupno,
			     gro->trgroupobjno);
	    break;
	default:
	    break;
...
{
    return fprintf (gl2ps->stream, "5 0 obj\n" "%d\n" "endobj\n", val);
}
...
static int
gl2psPrintPDFShaderStreamDataCoord (GL2PSvertex * vertex,
...
static int
gl2psPrintPDFShaderStreamData (GL2PStriangle * triangle,
			       GLfloat dx, GLfloat dy,
			       GLfloat xmin, GLfloat ymin,
			       size_t (*action) (unsigned long data,
						 size_t size), int gray)
{
    int i, offs = 0;
...
static void
gl2psPDFRectHull (GLfloat * xmin, GLfloat * xmax,
		  GLfloat * ymin, GLfloat * ymax, GL2PStriangle * triangles, int cnt)
{
    int i, j;
...
gl2psPrintPDFShader (int obj, GL2PStriangle * triangles, int size, int gray)
{
    ...
    offs += fprintf (gl2ps->stream,
		     ...
		     (gray) ? gray : 8, xmin, xmax, ymin, ymax, (gray) ? "" : "0 1 0 1");
...
gl2psPrintPDFShaderExtGS (int obj, int childobj)
{
    ...
    offs += fprintf (gl2ps->stream, "%d 0 obj\n" "<<\n", obj);
...
gl2psPrintPDFPixmap (int obj, int childobj, GL2PSimage * im, int gray)
{
	...
	if (Z_OK == gl2psDeflate ()
	    ...
	    offs += fprintf (gl2ps->stream,
			     ...
			     ">>\n" "stream\n", (int) gl2ps->compress->destLen);
	    offs +=
		gl2ps->compress->destLen * fwrite (gl2ps->compress->dest,
...
gl2psPDFgroupListWriteObjects (int entryoffs)
{
	...
	case GL2PS_TRIANGLE:
		...
		offs +=
		    gl2psPrintPDFShaderSimpleExtGS (gro->gsobjno,
...
static void
gl2psSVGGetCoordsAndColors (int n, GL2PSvertex * verts, GL2PSxyz * xyz, GL2PSrgba * rgba)
{
    int i, j;
...
gl2psPrintSVGPixmap (GLfloat x, GLfloat y, GL2PSimage * pixmap)
{
    ...
    gl2psListDelete (png);
#else
    (void)x;(void)y;(void)pixmap; // mask unused variables
...
gl2psPrintPGFPrimitive (void *data)
{
	...
	fprintf (gl2ps->stream,
		 ...
		 prim->verts[0].xyz[1] - 0.5 * prim->width, prim->width, prim->width);
	break;
    case GL2PS_LINE:
	gl2psPrintPGFColor (prim->verts[0].rgba);
	if (gl2ps->lastlinewidth != prim->width) {
...
gl2psPrintPrimitives (void)
{
    ...
    Progress_Window (-1, "Processing Display List", 100);
...
GL2PSDLL_API GLint
gl2psBeginPage (const char *title, const char *producer,
...
GL2PSDLL_API GLint
gl2psTextOpt (const char *str, const char *fontname,
	      GLshort fontsize, GLint alignment, GLfloat angle)
{
...
gl2psDrawPixels (GLsizei width, GLsizei height,
		 GLint xorig, GLint yorig, GLenum format, GLenum type, const void *pixels)
{
    int size, i;
...
GL2PSDLL_API GLint
gl2psDrawImageMap (GLsizei width, GLsizei height,
		   const GLfloat position[3], const unsigned char *imagemap)
{
    int size, i;
